...
import { startTestServer, stopTestServer, TestServerInstance } from '../helpers/testServer';
...
describe('再接続機能 - 統合テスト', () => {
  ...
  describe('IT-01: 基本的な再接続フロー', () => {
    it('Socket切断後、5秒以内に再接続できる', async () => {
      ...
      const joinPromise = new Promise<void>((resolve) => {
        client1.on('roomCreated', (data: { playerId: string; roomId: string }) => {
      ...
      client1.emit('createRoom', { hostName: 'TestPlayer1', smallBlind: 10, bigBlind: 20 });
      await joinPromise;
  ...
  describe('IT-03: グレースピリオド超過後の再接続失敗', () => {
    it('7秒後の再接続が失敗する（グレースピリオド5秒超過）', async () => {
      let playerId: string = '';
      let roomId: string = '';
  ...
  describe('IT-04: 複数プレイヤーの再接続', () => {
    it('複数プレイヤーが独立して再接続できる', async () => {
      ...
      const player1JoinPromise = new Promise<void>((resolve) => {
        client1.on('roomCreated', (data: { playerId: string; roomId: string }) => {
      ...
      client1.emit('createRoom', { hostName: 'Player1', smallBlind: 10, bigBlind: 20 });
      ...
      const player2JoinPromise = new Promise<void>((resolve) => {
        client2.on('joinedRoom', (data: { playerId: string }) => {
      ...
      const player1GameStatePromise = new Promise<any>((resolve) => {
        client1.on('gameState', resolve);
      });
      ...
      const player2GameStatePromise = new Promise<any>((resolve) => {
